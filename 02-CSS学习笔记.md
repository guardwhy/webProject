...
3D就是一个立体, 有宽度和高度, 还有厚度。默认情况下所有的元素都是呈2D展现的。
```
...
​	<img src="https://guardwhy.oss-cn-beijing.aliyuncs.com/img/前端/CSS/Test2/39-CSS.png" style="zoom: 67%;" />
...
*1搞一个容器, 里面放三个盒子*，*设置两侧盒子的宽度(固定)*。

*2设置中间盒子的宽度等于容器的宽度(100%)*，*设置容器的 padding等于两侧盒子的宽度*。

3让三个盒子都在同一个方向上浮动*，*设置左边盒子的margin-left=-100%*。

*4通过定位调整左边的盒子, 让左边的盒子不要盖住中间的区域*，*设置右边盒子的margin-left=-自身的宽度*。

5.*通过定位调整右边的盒子, 让右边的盒子不要盖住中间的区域*，*给容器设置一个最小的宽度, 防止缩小后变形*。
...
*1.搞一个容器, 里面放三个盒子*，*设置两侧盒子的宽度(固定)*。

*2.设置中间盒子的宽度等于容器的宽度(100%)*，*让三个盒子都在同一个方向上浮动*。

*3.给中间的盒子添加一个子盒子*，*给子盒子设置margin 0 两侧盒子的宽度*。

*4.由于是给子盒子设置margin,所以不会对父盒子排版产生任何影响*，*设置左边盒子的margin-left=-100%*。

*5. 设置右边盒子的margin-left=-自身的宽度*。
